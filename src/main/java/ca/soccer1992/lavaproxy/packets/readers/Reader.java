...
public abstract class Reader {
    ...
    public Packet read(ByteBuf buf, int ver) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        Packet p = getPacketFromInfo(ver, id).getDeclaredConstructor().newInstance();

        p.decode(buf);
        return p;
    }
    public int getPacketFromInfo(int ver, Class<? extends Packet> packet){
        return _getPacketFromInfo(ver, packet, serverDefinitions());
    }
    public Class<? extends Packet> getPacketFromInfo(int ver, int packetID){
        return _getPacketFromInfo(ver, packetID, serverDefinitions());
    }

    public int getPacketFromInfoClient(int ver, Class<? extends Packet> packet){
        return _getPacketFromInfo(ver, packet, clientDefinitions());
    }
    public Class<? extends Packet> getPacketFromInfoClient(int ver, int packetID){
        return _getPacketFromInfo(ver, packetID, clientDefinitions());
    }

    public int _getPacketFromInfo(int ver, Class<? extends Packet> packet, Map<Class<? extends Packet>, List<DefinitionPair>> definitions){
        DefinitionPair oldEntry = null;
        int correct = 0;
        for (DefinitionPair pair : definitions.get(packet)){
            if (oldEntry != null){
                if (pair.protocol()>ver){
    ...
    public Class<? extends Packet> _getPacketFromInfo(int ver, int packetID, Map<Class<? extends Packet>, List<DefinitionPair>> definitions){
        DefinitionPair oldEntry = null;
